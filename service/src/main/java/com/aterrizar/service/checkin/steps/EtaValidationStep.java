...
public class EtaValidationStep implements Step {
    ...
    public boolean when(Context context) {
        ...
        boolean isCountryEnabled = enabledCountries.contains(sessionCountry) ||
                ("GB".equals(sessionCountry) && enabledCountries.contains("UK"));

        boolean hasPassport = Optional.ofNullable(context.session().userInformation())
    ...
    public StepResult onExecute(Context context) {
        ...
        try {
            EtaResponse response = homeOfficeHttpClient.validateEta(new EtaRequest(passportNumber, destinationCode));
            System.out.println("DEBUG: El estatus que devolvió WireMock es: [" + response.status() + "]");
            ...
            System.err.println("FALLO REAL - Sesion: [" + context.session().sessionId() + "] - Pasaporte: [" + passportNumber + "]");
            throw e;
        } catch (Exception e) {
            throw new IllegalStateException("Fallo técnico en sesión [" + context.session().sessionId() + "]: " + e.getMessage(), e);
        }/*catch (IllegalStateException e) {
            throw e;
        } catch (Exception e) {
